...
//driver
router.post('/forgotDriver', async (req, res) => {
    if (req.body.email) {
        ...
        try {
            ...
            html: `<h2>${user.fullname.first} Thank for using this platform</h2>
                <p>Reset your password</p>
                <a href = "http://${req.headers.host}/resetDriver?token=${user.emailToken}">Verify</a>
            `
            }
...
router.get('/resetDriver', async (req, res) => {
    try {
        const token = req.query.token
...
router.post('/resetDriver', async (req, res) => {
    try {
        const token = req.query.token
        const user = await Driver.findOne({ emailToken: token })
